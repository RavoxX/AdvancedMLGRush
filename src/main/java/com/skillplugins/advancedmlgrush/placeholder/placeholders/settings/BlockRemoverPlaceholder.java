...
@Singleton
public class BlockRemoverPlaceholder extends Placeholder {

    private final SQLDataCache sqlDataCache;

    @Inject
    public BlockRemoverPlaceholder(final @NotNull SQLDataCache sqlDataCache) {
        this.sqlDataCache = sqlDataCache;
    }

    @Override
    public String identifier() {
        return "%settings_block_remover%";
    }
    ...
    public String onRequest(final @NotNull Optional<Player> optionalPlayer) {
        ...
        return BlockRemover.fromId(sqlDataCache.getSQLData(player).getSettingsBlockRemover()).getDisplayName();
    }
}
